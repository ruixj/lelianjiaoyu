...
function buddyboss_scripts_styles() {
	...
	if ( !is_admin() ) {
		...
		if ( boss_get_option( 'boss_minified_js' ) ) {
			...
			wp_register_script( 'buddyboss-main', get_template_directory_uri() . '/js/buddyboss.js', array( 'jquery', 'plupload', 'moxie' ), $boss_version );
			...
			case 'trackback' :
				...
				( $comment->user_id === $post->post_author ) ? '<span> ' . __( 'Post author', 'boss' ) . '</span>' : ''
							...
							<footer class="comment-meta">
								<?php
								printf( '<a href="%1$s"><time datetime="%2$s">%3$s 以前</time></a>', esc_url( get_comment_link( $comment->comment_ID ) ), get_comment_time( 'c' ),
